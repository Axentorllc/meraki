...
def send_message(sales_order_name):
    ...
    image = sales_order.cake_sample_image
...
def format_message(sales_order):
    ...
    basics = format_basics(sales_order.cake_basic_information)
    words = sales_order.writing_on_cake or ""
    customer_address = get_customer_contact(customer_name) or ""
    toppers = ""
    decors = ""
    ...
    if (sales_order.grand_total):
        full_msg = (full_msg +
                    ...
                    "\n  Payment: " + str(sales_order.paid_amount) +
                    "\n\n *Writting on cake*: " + words +
                    "\n\n Toppers: " + toppers +
                    "\n Decor: " + decors +                    
                    "\n\n Notes: \n " + (str(sales_order.notes) or "") ) # notes

    #age = format_basics(sales_order.cake_basic_information.age)
    #frappe.msgprint(_(age))
